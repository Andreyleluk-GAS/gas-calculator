...
const App = () => {
  ...
  const passResults = useMemo(() => {
    ...
    return {
      ...
      kmCostB: formatKm(kmCostB),
      kmCostP: formatKm(kmCostP),
      kmCostM: formatKm(kmCostM),
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center p-1 md:p-8 font-sans overflow-x-hidden text-slate-900">
        <div className="w-full max-w-lg lg:max-w-4xl flex flex-col gap-2 md:gap-3 text-slate-900">
          <header className="flex flex-col items-center text-center bg-white p-3 md:p-4 rounded-2xl shadow-sm border border-slate-200 w-full relative mt-2">
            <button onClick={() => window.history.back()} className="hidden md:block absolute left-2 top-2 md:left-3 md:top-4 p-2 text-slate-600 hover:bg-slate-50 rounded-full"><ChevronLeft size={20} className="md:w-6 md:h-6" /></button>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-2 md:gap-3 w-full mb-3 text-slate-900">
            {/* БЕНЗИН */}
            <div className="bg-white rounded-2xl p-2.5 md:p-5 shadow-sm border-t-4 border-amber-400 flex flex-col justify-between text-slate-900">
                <div>
                ...
                <p className="text-[9px] md:text-[10px] font-semibold text-slate-400 pt-2 border-t border-slate-100 mt-3 md:mt-4">Эталон для сравнения</p>
            </div>
            ...
            <div className="bg-white rounded-2xl p-2.5 md:p-5 shadow-sm border-t-4 border-emerald-500 bg-emerald-50/30 flex flex-col justify-between text-slate-900">
                ...
                <p className="text-[9px] md:text-[11px] font-bold text-emerald-600 pt-2 border-t border-emerald-200 mt-3 md:mt-4">Экономия в год: +{formatMoney(passResults.saveYearP)}</p>
            </div>
      ...
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4 font-sans text-slate-900">
        <div className="max-w-4xl w-full text-slate-900">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8 text-slate-900">
            ...
            <div onClick={() => { setTruckSubMode('REMOT'); navigateTo('TRUCK_INPUTS', { truckSubMode: 'REMOT' }); }} className="bg-white p-6 rounded-[2rem] border-2 border-slate-100 hover:border-amber-400 cursor-pointer shadow-sm transition-all group">
              ...
              <div className={`grid grid-cols-1 ${systemType === 'cng' ? 'md:grid-cols-2' : ''} gap-2 md:gap-6 mb-3 md:mb-8 font-sans`}>
                <div className={`bg-gradient-to-br ${truckTheme.gradient} text-white p-4 md:p-8 rounded-2xl md:rounded-[2rem] shadow-xl flex flex-row justify-between relative overflow-hidden font-sans`}>
                  ...
                  <div className="absolute right-[10px] top-[10px] bottom-[10px] flex items-center justify-end w-2/5 md:w-1/3 select-none">
                    <img src="/logo.png" alt="Logo" className="h-full w-auto object-contain object-right opacity-30" onError={(e)=>{e.target.style.display='none'}} />
                  </div>
                </div>
                {systemType === 'cng' && (
                  <div className="bg-white border-2 border-blue-100 p-4 md:p-8 rounded-2xl md:rounded-[2rem] shadow-lg flex flex-col justify-between relative overflow-hidden text-slate-900 font-sans text-slate-900 text-slate-900">
                    ...
                    <div className="absolute right-[10px] bottom-[10px] flex items-end justify-end max-h-[35%] w-[28%] select-none opacity-40">
                ...
                <div className="flex justify-between mt-2 text-[8px] md:text-[10px] font-bold uppercase tracking-tight text-slate-900 text-slate-900">
                  {truckSubMode === 'REMOT' ? (<span className={`${truckTheme.textDark} font-bold text-slate-900`}>100% {gasName} Метан</span>) : (
                    <>
                      ...
                      <span className={`${truckTheme.textDark} font-bold text-slate-900 text-slate-900`}>{gasName}: {formatMoney(truckSummary.totalG * (truckInputs.substitutionRate/100))}</span>
                    </>
                  )}
                </div>
              </div>
