      ...
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4 font-sans text-slate-900 relative overflow-hidden">
        <div className="max-w-xl w-full relative z-10 flex flex-col items-center text-slate-900">
          ...
          <div className="grid grid-cols-1 gap-4 w-full">
            <button onClick={() => navigateTo('TRUCK_HOME')} className="bg-white/80 backdrop-blur-md p-6 rounded-[2rem] border-2 border-white/50 hover:border-blue-500 shadow-sm hover:shadow-xl transition-all group flex items-center gap-6 text-left">
              <div className="bg-blue-50 w-16 h-16 rounded-2xl flex items-center justify-center text-blue-600 group-hover:scale-105 transition-transform shrink-0"><Truck size={32} /></div>
            ...
            <button onClick={() => navigateTo('PASSENGER_CALC')} className="bg-white/80 backdrop-blur-md p-6 rounded-[2rem] border-2 border-white/50 hover:border-emerald-500 shadow-sm hover:shadow-xl transition-all group flex items-center gap-6 text-left">
              <div className="bg-emerald-50 w-16 h-16 rounded-2xl flex items-center justify-center text-emerald-600 group-hover:scale-105 transition-transform shrink-0"><Car size={32} /></div>
              <div><h3 className="text-xl font-bold">Для легкового ТС</h3><p className="text-slate-500 text-xs font-bold uppercase tracking-tighter">Пропан и Метан (ГБО)</p></div>
              <ChevronRight className="ml-auto text-slate-300" />
            </button>
          </div>
          <AppFooter />
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center p-1 md:p-8 font-sans overflow-x-hidden text-slate-900">
        <div className="w-full max-w-lg lg:max-w-4xl flex flex-col gap-2 md:gap-3">
          <header className="flex flex-col items-center text-center bg-white py-4 md:py-6 px-4 rounded-2xl shadow-sm border border-slate-200 w-full relative mt-2">
            ...
            <div className="flex items-center gap-2 mt-1 md:mt-2">
                <span className="bg-blue-100 text-blue-700 px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-tight tracking-tight">Свердловскстат</span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-2 md:gap-3 w-full mb-3 text-slate-900">
            {/* БЕНЗИН */}
            <div className="bg-white rounded-2xl p-2.5 md:p-5 shadow-sm border-t-4 border-amber-400 flex flex-col justify-between">
                <div>
                  ...
                  <div className="space-y-3 md:space-y-4">
                      ...
                      {/* Горизонтальный блок результатов */}
                      <div className="flex items-end justify-between border-t border-slate-100 pt-3">
                         <div className="flex flex-col">
                            <p className="text-[9px] md:text-[10px] font-bold text-slate-400 uppercase mb-1">Затраты / мес</p>
                            <p className="text-2xl md:text-3xl font-black leading-none text-slate-900">{formatMoney(passResults.costB)}</p>
                         </div>
                         <div className="flex flex-col text-right">
                            <p className="text-[9px] md:text-[10px] font-bold text-slate-400 uppercase mb-1">Стоимость 1 км</p>
                            <p className="text-lg md:text-xl font-bold text-slate-600 leading-none">{passResults.kmCostB} Р/км</p>
                         </div>
                      </div>
                  </div>
                </div>
                <p className="text-[9px] md:text-[10px] font-semibold text-slate-400 pt-2 border-t border-slate-100 mt-4 text-center italic leading-none">Эталон для сравнения</p>
            </div>

            {/* ПРОПАН */}
            <div className="bg-white rounded-2xl p-2.5 md:p-5 shadow-sm border-t-4 border-emerald-500 bg-emerald-50/30 flex flex-col justify-between">
                <div>
                  <div className="flex items-center gap-2 mb-1.5 md:mb-4">
                  ...
                  <div className="space-y-3 md:space-y-4">
                      <div className="flex justify-between items-center bg-white p-2 rounded-xl border border-emerald-100 text-slate-900">
                      ...
                      {/* Горизонтальный блок результатов */}
                      <div className="flex items-end justify-between border-t border-emerald-100 pt-3">
                         <div className="flex flex-col">
                            <p className="text-[9px] md:text-[10px] font-bold text-emerald-800/60 uppercase mb-1">Затраты / мес</p>
                            <p className="text-2xl md:text-3xl font-black leading-none text-emerald-900">{formatMoney(passResults.costP)}</p>
                         </div>
                         <div className="flex flex-col text-right">
                            <p className="text-[9px] md:text-[10px] font-bold text-emerald-800/60 uppercase mb-1">Стоимость 1 км</p>
                            <p className="text-lg md:text-xl font-bold text-emerald-700 leading-none">{passResults.kmCostP} Р/км</p>
                         </div>
                      </div>
                  </div>
                </div>
                <p className="text-[9px] md:text-[11px] font-bold text-emerald-600 pt-2 border-t border-emerald-200 mt-4 text-center tracking-tight leading-none text-center">Экономия в год: +{formatMoney(passResults.saveYearP)}</p>
            </div>

            {/* МЕТАН */}
            <div className="bg-white rounded-2xl p-2.5 md:p-5 shadow-sm border-t-4 border-blue-500 bg-blue-50/30 flex flex-col justify-between text-slate-900">
                <div>
                  <div className="flex items-center gap-2 mb-1.5 md:mb-4">
                  ...
                  <div className="space-y-3 md:space-y-4">
                      <div className="flex justify-between items-center bg-white p-2 rounded-xl border border-blue-100 text-slate-900">
                          <label className="text-[9px] md:text-[10px] font-bold text-blue-800 uppercase">Цена / м³</label>
                          <div className="flex items-baseline gap-1">
                               <input type="number" name="priceMethane" value={passInputs.priceMethane} onChange={handlePassInputChange} className="w-14 bg-transparent text-right font-bold text-lg outline-none text-slate-900" />
                      ...
                      <div className="flex items-end justify-between border-t border-blue-100 pt-3">
                         <div className="flex flex-col">
                            <p className="text-[9px] md:text-[10px] font-bold text-blue-800/60 uppercase mb-1">Затраты / мес</p>
                            <p className="text-2xl md:text-3xl font-black leading-none text-blue-900">{formatMoney(passResults.costM)}</p>
                         </div>
                         <div className="flex flex-col text-right">
                            <p className="text-[9px] md:text-[10px] font-bold text-blue-800/60 uppercase mb-1">Стоимость 1 км</p>
                            <p className="text-lg md:text-xl font-bold text-blue-700 leading-none">{passResults.kmCostM} Р/км</p>
                         </div>
                      </div>
                  </div>
                </div>
                <p className="text-[9px] md:text-[11px] font-bold text-blue-600 pt-2 border-t border-blue-200 mt-4 text-center tracking-tight leading-none text-center">Экономия в год: +{formatMoney(passResults.saveYearM)}</p>
            </div>
          </div>
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4 font-sans text-slate-900">
        <div className="max-w-4xl w-full">
          <button onClick={() => window.history.back()} className="hidden md:flex items-center gap-1 mb-6 text-slate-900 font-bold text-sm hover:opacity-70 text-slate-900 text-slate-900"><ChevronLeft size={20} /> Назад</button>
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col p-2 font-sans text-slate-900">
        <div className="flex-1 flex flex-col items-center justify-center max-w-xl mx-auto w-full">
          <button onClick={() => window.history.back()} className="hidden md:flex items-center gap-1 mb-4 self-start text-slate-900 font-bold text-xs"><ChevronLeft size={18} /> Назад</button>
          ...
          <div className="bg-white rounded-[2rem] shadow-xl p-5 w-full border border-slate-200">
            <div className="space-y-4">
              ...
              <div className="p-4 bg-red-50 rounded-2xl border border-red-100">
                <div className="flex items-center gap-2 mb-3 text-red-900 font-bold uppercase text-[10px] text-slate-900 font-bold"><Fuel size={14} /> Дизельное топливо</div>
      ...
      <div className="min-h-screen bg-slate-50 p-1.5 md:p-8 font-sans text-slate-900 overflow-x-hidden">
        <div className="max-w-6xl mx-auto text-slate-900">
          <header className="mb-2 md:mb-4 flex items-center justify-between print-hidden">
            <button onClick={() => window.history.back()} className="hidden md:flex items-center gap-1 px-4 py-2 bg-white border border-slate-300 rounded-xl text-xs font-bold text-slate-800 shadow-sm font-sans"><ChevronLeft size={14} /> Назад</button>
          ...
          <div className="bg-white p-3 md:p-10 rounded-2xl md:rounded-[2.5rem] shadow-sm border border-slate-200 relative overflow-hidden text-slate-900">
            {/* Фоновый логотип (подложка) удален согласно инструкции */}
            <div className="relative z-10 text-slate-900">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 gap-1.5 md:gap-3 mb-3 md:mb-8 font-bold text-slate-900">
                <div className="p-2 md:p-3 bg-slate-50 rounded-xl border border-slate-200 font-sans text-slate-900"><div className="text-slate-700 text-[8px] md:text-[10px] uppercase mb-0.5 md:mb-1 font-bold text-slate-900 text-slate-900">Пробег</div><div className="font-bold text-[10px] md:text-base">{(truckSubMode === 'REMOT' ? remotInputs.monthlyMileage : truckInputs.monthlyMileage).toLocaleString()} км/мес</div></div>
                <div className="p-2 md:p-3 bg-slate-50 rounded-xl border border-slate-200 font-sans text-slate-900 text-slate-900"><div className="text-slate-700 text-[8px] md:text-[10px] uppercase mb-0.5 md:mb-1 font-bold text-slate-900">Расход ДТ</div><div className="font-bold text-[10px] md:text-base">{truckSummary.qD_base} л/100км</div></div>
              ...
              <div className={`grid grid-cols-1 ${systemType === 'cng' ? 'md:grid-cols-2' : ''} gap-2 md:gap-6 mb-3 md:mb-8 font-sans`}>
                <div className={`bg-gradient-to-br ${truckTheme.gradient} text-white p-4 md:p-8 rounded-2xl md:rounded-[2rem] shadow-xl flex flex-row justify-between relative overflow-hidden font-sans`}>
                  <div className="relative z-10 flex flex-col justify-between w-2/3 md:w-3/4 text-white text-white">
                    <div><div className="text-[9px] md:text-xs font-bold uppercase tracking-wider opacity-90 font-sans">Экономия (Базовый)</div><div className="text-xl md:text-5xl font-bold mb-2 md:mb-4 leading-tight font-sans text-white">{formatMoney(truckSummary.savings)}</div></div>
                    <div className="flex gap-2 md:gap-3 flex-wrap text-white text-white text-white text-white text-white"><div className="bg-white/20 px-2 py-0.5 md:px-3 md:py-1.5 rounded-lg text-[8px] md:text-[10px] font-bold uppercase">{formatMoney(truckSummary.monthlySav)} / мес</div><div className="bg-white/20 px-2 py-0.5 md:px-3 md:py-1.5 rounded-lg text-[8px] md:text-[10px] font-bold uppercase font-sans text-white text-white text-white">- {Math.round((truckSummary.savings / (truckSummary.totalD || 1)) * 100)}% затрат</div></div>
                  </div>
                  <div className="absolute right-[10px] top-[10px] bottom-[10px] flex items-center justify-end w-2/5 md:w-1/3 select-none text-white text-white text-white">
                    <img src="/logo.png" alt="Logo" className="h-full w-auto object-contain object-right opacity-30 text-white" onError={(e)=>{e.target.style.display='none'}} />
                  </div>
                </div>
                {systemType === 'cng' && (
                  <div className="bg-white border-2 border-blue-100 p-4 md:p-8 rounded-2xl md:rounded-[2rem] shadow-lg flex flex-col justify-between relative overflow-hidden text-slate-900 font-sans text-slate-900 text-slate-900 text-slate-900">
                    ...
                    <div className="relative z-10 w-2/3 md:w-3/4 mt-2 font-bold text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900">
                        <div><div className="text-[9px] md:text-xs font-bold text-slate-700 mb-0.5 md:mb-1 flex items-center gap-1 uppercase tracking-wider font-sans text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900"><Tag size={10} className="text-blue-600 font-sans text-blue-600" /> Со скидкой на метан 20%</div><div className="text-xl md:text-5xl font-bold text-blue-900 mb-2 md:mb-4 leading-tight text-blue-900 text-blue-900 text-blue-900 text-blue-900">{formatMoney(truckSummary.savingsDiscounted)}</div></div>
                        <div className="flex gap-2 md:gap-3 flex-wrap text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900"><div className="bg-blue-50 text-blue-900 px-2 py-0.5 md:px-3 md:py-1.5 rounded-lg text-[8px] md:text-[10px] font-bold border border-blue-200 uppercase font-sans text-blue-900 text-blue-900 text-blue-900 text-blue-900 text-blue-900">{formatMoney(truckSummary.monthlySavDiscounted)} / мес</div><div className="bg-blue-50 text-blue-900 px-2 py-0.5 md:px-3 md:py-1.5 rounded-lg text-[8px] md:text-[10px] font-bold border border-blue-200 uppercase font-sans text-blue-900 text-blue-900 text-blue-900 text-blue-900 text-blue-900 text-blue-900">- {Math.round((truckSummary.savingsDiscounted / (truckSummary.totalD || 1)) * 100)}% затрат</div></div>
                    </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2 md:gap-6 mb-3 md:mb-8 font-sans font-bold text-slate-900 text-slate-900 text-slate-900 text-slate-900">
                <div className="border border-red-200 rounded-xl md:rounded-[2rem] p-3 md:p-6 bg-red-50/30 text-slate-900 text-slate-900 text-slate-900 text-slate-900">
                  ...
                  <div className="space-y-1.5 md:space-y-3 text-slate-900">
                    <div className="flex justify-between text-[10px] md:text-sm font-bold opacity-80 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900"><span>Расход на 100км:</span><span>{truckSummary.qD_base} л</span></div>
                    <div className="flex justify-between text-[10px] md:text-sm font-bold opacity-80 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900"><span>Стоимость 1 км:</span><span>{truckSummary.kmD?.toFixed(2)} ₽</span></div>
                    <div className="border-t border-red-200 pt-2 md:pt-3 flex justify-between font-bold text-red-700 text-sm md:text-2xl leading-none text-red-700 text-red-700 text-red-700 text-red-700">
                      <span className="text-[8px] md:text-xs uppercase self-center font-bold text-red-600 tracking-tighter text-red-600 text-red-600 text-red-600 text-red-600">ИТОГО ЗА ГОД:</span>
                      <span className="font-black text-red-800 text-red-800 text-red-800 text-red-800 text-red-800">{formatMoney(truckSummary.totalD)}</span>
                    </div>
                  </div>
                </div>
                <div className={`border ${truckTheme.border} rounded-xl md:rounded-[2rem] p-3 md:p-6 ${truckTheme.bg}/30 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900`}>
                  <div className={`${truckTheme.textDark} font-bold text-[10px] md:text-xs uppercase mb-2 md:mb-4 flex items-center gap-2 font-sans`}>{isLng ? <Flame size={12}/> : <Gauge size={12}/>} {truckSubMode === 'REMOT' ? `На газе (${gasName} 100%)` : `Газодизель (${truckInputs.substitutionRate}% замещения)`}</div>
                  <div className="space-y-1.5 md:space-y-3 font-bold text-slate-900 text-slate-900 text-slate-900 text-slate-900">
                    ...
                    <div className={`border-t ${truckTheme.border} pt-2 md:pt-3 flex justify-between font-bold ${truckTheme.textDark} text-sm md:text-2xl leading-none text-slate-900 text-slate-900 text-slate-900 text-slate-900`}>
                      <span className="text-[8px] md:text-xs uppercase self-center font-bold tracking-tighter text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900">ИТОГО ЗА ГОД:</span>
                      <span className="font-black text-slate-900 text-slate-900 text-slate-900 text-slate-900">{formatMoney(truckSummary.totalG)}</span>
                    </div>
                  </div>
                </div>
              </div>
              <div className="bg-slate-100 p-3 md:p-6 rounded-xl md:rounded-[2.5rem] border border-slate-200 font-bold text-slate-900 text-slate-900 text-slate-900 text-slate-900">
                <h4 className="text-[8px] md:text-[10px] font-bold text-slate-500 uppercase mb-2 md:mb-6 flex items-center gap-2 tracking-widest font-sans text-slate-500 text-slate-500 text-slate-500 text-slate-500 text-slate-500"><BarChart3 size={12}/> Структура затрат</h4>
                <div className="h-6 md:h-10 w-full bg-slate-300 rounded-lg md:rounded-2xl overflow-hidden flex shadow-inner text-slate-900 text-slate-900 text-slate-900 text-slate-900">
                  {truckSubMode === 'REMOT' ? (<div className={`${truckTheme.button} h-full w-full text-slate-900 text-slate-900 text-slate-900 text-slate-900`}></div>) : (
                    <>
                      <div className="bg-red-600 h-full transition-all duration-1000 text-slate-900 text-slate-900 text-slate-900 text-slate-900" style={{ width: `${(100 - truckInputs.substitutionRate)}%` }}></div>
                      <div className={`${truckTheme.button} h-full transition-all duration-1000 text-slate-900 text-slate-900 text-slate-900 text-slate-900`} style={{ width: `${truckInputs.substitutionRate}%` }}></div>
                    </>
                  )}
                </div>
                <div className="flex justify-between mt-2 text-[8px] md:text-[10px] font-bold uppercase tracking-tight text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900">
                  {truckSubMode === 'REMOT' ? (<span className={`${truckTheme.textDark} font-bold text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900`}>100% {gasName} Метан</span>) : (
                    <>
                      <span className="text-red-700 font-bold text-red-700 text-red-700 text-red-700 text-red-700 text-red-700 text-red-700 text-red-700 text-red-700">ДИЗЕЛЬ: {formatMoney(truckSummary.totalG * (1 - truckInputs.substitutionRate/100))}</span>
                      <span className={`${truckTheme.textDark} font-bold text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900`}>{gasName}: {formatMoney(truckSummary.totalG * (truckInputs.substitutionRate/100))}</span>
                    </>
                  )}
                </div>
              </div>
