...
const App = () => {
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-2 md:p-4 font-sans text-slate-900 relative overflow-hidden">
        <div className="max-w-xl w-full relative z-10 flex flex-col items-center">
          <div className="bg-white/80 backdrop-blur-md p-6 rounded-[2rem] border border-slate-200 shadow-sm flex items-center justify-center mb-4 md:mb-8 w-full">
            <div className="h-12 flex items-center justify-center text-red-600 font-black text-2xl italic tracking-tighter">
              ELITE<span className="text-slate-900">GAS</span>
            </div>
          </div>
  ...
  if (currentScreen === 'PASSENGER_CALC') {
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center p-1 md:p-8 font-sans overflow-x-hidden">
        <div className="w-full max-w-lg lg:max-w-4xl flex flex-col gap-1 md:gap-3">
          <header className="flex flex-col items-center text-center bg-white py-1.5 md:py-6 px-4 rounded-2xl shadow-sm border border-slate-200 w-full relative mt-0.5 md:mt-2">
          ...
          <section className="grid grid-cols-2 gap-1 md:gap-3 w-full">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-1 md:gap-3 w-full mb-1.5 md:mb-3">
            {/* БЕНЗИН */}
                  ...
                  <div className="space-y-2 md:space-y-4">
                      ...
                      <div className="flex items-end justify-between border-t border-slate-100 pt-2 md:pt-3">
                         <div className="flex flex-col">
                            ...
                            <p className="text-2xl md:text-3xl font-black leading-none">{formatMoney(passResults.costB)}</p>
                         </div>
            ...
            {/* ПРОПАН */}
            <div className="bg-white rounded-2xl p-2 md:p-5 shadow-sm border-t-4 border-emerald-500 bg-emerald-50/30 flex flex-col justify-between">
                <div>
                  <div className="flex items-center gap-2 mb-1.5 md:mb-4">
                ...
                <p className="text-[10px] md:text-[13px] font-bold text-emerald-600 pt-1.5 border-t border-emerald-200 mt-2.5 md:mt-4 text-center tracking-tight leading-none">Экономия в год: +{formatMoney(passResults.saveYearP)}</p>
            </div>

            {/* МЕТАН */}
            <div className="bg-white rounded-2xl p-2 md:p-5 shadow-sm border-t-4 border-blue-500 bg-blue-50/30 flex flex-col justify-between">
                <div>
                  <div className="flex items-center gap-2 mb-1.5 md:mb-4">
                  ...
                  <div className="space-y-2 md:space-y-4">
                      ...
                      <div className="flex items-end justify-between border-t border-blue-100/50 pt-2 md:pt-3">
                         <div className="flex flex-col">
                ...
                <p className="text-[10px] md:text-[13px] font-bold text-blue-600 pt-1.5 border-t border-blue-200 mt-2.5 md:mt-4 text-center tracking-tight leading-none">Экономия в год: +{formatMoney(passResults.saveYearM)}</p>
            </div>
          </div>
          ...
          <div className="mt-auto">
            <div className="flex items-center justify-center gap-2 text-[10px] md:text-[11px] text-slate-900 font-bold bg-white px-4 py-2 rounded-full border shadow-sm mx-auto w-fit mb-4">
                ...
                <button onClick={() => setIsPassSettingsOpen(true)} className="p-1 hover:bg-slate-100 rounded-lg text-slate-500 transition-colors"><Settings size={14} /></button>
            </div>
            <AppFooter />
          </div>
        </div>
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4 font-sans">
        <div className="max-w-4xl w-full">
          <button onClick={() => window.history.back()} className="hidden md:flex items-center gap-1 mb-6 text-slate-500 font-bold text-sm hover:text-slate-900 transition-colors"><ChevronLeft size={20} /> Назад</button>
          <div className="text-center mb-8">
            <h1 className="text-2xl md:text-4xl font-extrabold mb-2">Грузовой транспорт</h1>
            <p className="text-slate-600 font-medium uppercase tracking-tight">Выберите технологию переоборудования</p>
          </div>
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col p-2 font-sans">
        <div className="flex-1 flex flex-col items-center justify-center max-w-xl mx-auto w-full">
          <button onClick={() => window.history.back()} className="hidden md:flex items-center gap-1 mb-4 self-start text-slate-500 font-bold text-xs hover:text-slate-900"><ChevronLeft size={18} /> Назад</button>
          ...
          <div className="bg-white rounded-[2rem] shadow-xl p-5 w-full border border-slate-200">
            <div className="space-y-4">
              ...
              <div className="p-4 bg-red-50 rounded-2xl border border-red-100">
                <div className="flex items-center gap-2 mb-3 text-red-900 font-bold uppercase text-[10px] tracking-tight"><Fuel size={14} /> Дизельное топливо</div>
              ...
              <div className={`grid ${!isRem ? 'grid-cols-2' : 'grid-cols-1'} gap-3`}>
                <div>
                  <label className="block text-[10px] font-bold text-slate-800 uppercase mb-1 tracking-tight">Пробег (км/мес)</label>
                  <input type="number" name="monthlyMileage" value={activeInp.monthlyMileage} onChange={(e) => handleTruckInputChange(e, isRem)} className="w-full p-3 border border-slate-300 rounded-xl font-bold text-sm" />
                </div>
                {!isRem && (
                  <div>
                    <label className="block text-[10px] font-bold text-slate-800 uppercase mb-1 tracking-tight">% замещения ДТ</label>
      ...
      <div className="min-h-screen bg-slate-50 p-1.5 md:p-8 font-sans text-slate-900 overflow-x-hidden">
        <div className="max-w-6xl mx-auto">
          <header className="mb-2 md:mb-4 flex items-center justify-between print-hidden">
            <button onClick={() => window.history.back()} className="hidden md:flex items-center gap-1 px-4 py-2 bg-white border border-slate-300 rounded-xl text-xs font-bold text-slate-800 shadow-sm">
                <ChevronLeft size={14} />
                <span>Назад</span>
            </button>
          ...
          <div className="bg-white p-3 md:p-10 rounded-2xl md:rounded-[2.5rem] shadow-sm border border-slate-200 relative overflow-hidden">
            <div className="relative z-10">
              <div className="mb-3 md:mb-8 text-center border-b border-slate-100 pb-2 md:pb-6">
                ...
                <p className="text-[8px] md:text-xs text-slate-400 mt-1 font-semibold uppercase">Период расчета: 12 месяцев</p>
              </div>
              <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 gap-1.5 md:gap-3 mb-3 md:mb-8">
                <div className="p-2 md:p-3 bg-slate-50 rounded-xl border border-slate-200"><div className="text-slate-400 text-[8px] md:text-[10px] uppercase mb-0.5 md:mb-1 font-bold">Пробег</div><div className="font-bold text-[10px] md:text-base">{(truckSubMode === 'REMOT' ? remotInputs.monthlyMileage : truckInputs.monthlyMileage).toLocaleString()} км/мес</div></div>
                <div className="p-2 md:p-3 bg-slate-50 rounded-xl border border-slate-200"><div className="text-slate-400 text-[8px] md:text-[10px] uppercase mb-0.5 md:mb-1 font-bold">Расход ДТ</div><div className="font-bold text-[10px] md:text-base">{truckSummary.qD_base} л/100км</div></div>
                <div className="p-2 md:p-3 bg-slate-50 rounded-xl border border-slate-200"><div className="text-slate-400 text-[8px] md:text-[10px] uppercase mb-0.5 md:mb-1 font-bold">Цена ДТ</div><div className="font-bold text-[10px] md:text-base">{truckSubMode === 'REMOT' ? remotInputs.dieselPrice : truckInputs.dieselPrice} ₽</div></div>
                ...
                <div className="hidden md:block p-2 md:p-3 bg-slate-50 rounded-xl border border-slate-200"><div className="text-slate-400 text-[8px] md:text-[10px] uppercase mb-0.5 md:mb-1 font-bold tracking-tight">Коэф. расхода</div><div className="font-bold text-[10px] md:text-base">{truckSummary.gasCoef}</div></div>
              </div>
              <div className={`grid grid-cols-1 ${systemType === 'cng' ? 'md:grid-cols-2' : ''} gap-2 md:gap-6 mb-3 md:mb-8`}>
                <div className={`bg-gradient-to-br ${truckTheme.gradient} text-white p-4 md:p-8 rounded-2xl shadow-xl flex flex-row justify-between relative overflow-hidden`}>
                ...
                {systemType === 'cng' && (
                  <div className="bg-white border-2 border-blue-100 p-4 md:p-8 rounded-2xl md:rounded-[2rem] shadow-lg flex flex-col justify-between relative overflow-hidden">
                    ...
                    <div className="relative z-10 w-2/3 md:w-3/4 mt-2">
                        <div><div className="text-[9px] md:text-xs font-bold text-slate-400 mb-0.5 md:mb-1 flex items-center gap-1 uppercase tracking-wider"><Tag size={10} className="text-blue-600" /> Со скидкой на метан 20%</div><div className="text-xl md:text-5xl font-bold text-blue-900 mb-2 md:mb-4 leading-tight">{formatMoney(truckSummary.savingsDiscounted)}</div></div>
                        <div className="flex gap-2 md:gap-3 flex-wrap"><div className="bg-blue-50 text-blue-900 px-2 py-0.5 md:px-3 md:py-1.5 rounded-lg text-[8px] md:text-[10px] font-bold border border-blue-200 uppercase">{formatMoney(truckSummary.monthlySavDiscounted)} / мес</div><div className="bg-blue-50 text-blue-900 px-2 py-0.5 md:px-3 md:py-1.5 rounded-lg text-[8px] md:text-[10px] font-bold border border-blue-200 uppercase">- {Math.round((truckSummary.savingsDiscounted / (truckSummary.totalD || 1)) * 100)}% затрат</div></div>
                    </div>
                  </div>
                )}
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2 md:gap-6 mb-3 md:mb-8">
                <div className="border border-red-200 rounded-xl md:rounded-[2rem] p-3 md:p-6 bg-red-50/30">
                  ...
                  <div className="space-y-1.5 md:space-y-3">
                    <div className="flex justify-between text-[10px] md:text-sm opacity-80 font-medium"><span>Расход на 100км:</span><span>{truckSummary.qD_base} л</span></div>
                    <div className="flex justify-between text-[10px] md:text-sm opacity-80 font-medium"><span>Стоимость 1 км:</span><span>{truckSummary.kmD?.toFixed(2)} ₽</span></div>
                    <div className="border-t border-red-200 pt-2 md:pt-3 flex justify-between font-bold text-red-700 text-sm md:text-2xl leading-none">
                      <span className="text-[8px] md:text-xs uppercase self-center font-bold text-red-400 tracking-tighter">ИТОГО ЗА ГОД:</span>
                      <span className="font-black">{formatMoney(truckSummary.totalD)}</span>
                    </div>
                  </div>
                </div>
                <div className={`border ${truckTheme.border} rounded-xl md:rounded-[2rem] p-3 md:p-6 ${truckTheme.bg}/30`}>
                  <div className={`${truckTheme.textDark} font-bold text-[10px] md:text-xs uppercase mb-2 md:mb-4 flex items-center gap-2`}>{isLngMode?<Flame size={12}/>:<Gauge size={12}/>} {truckSubMode === 'REMOT' ? `На газе (${gasNameStr} 100%)` : `Газодизель (${truckInputs.substitutionRate}% замещения)`}</div>
                  <div className="space-y-1.5 md:space-y-3">
                    <div className="flex justify-between text-[10px] md:text-sm opacity-80 font-medium"><span>На 100км:</span><span>{truckSubMode === 'REMOT' ? `${truckSummary.qG_result} ${gasUnit}` : `${truckSummary.qD_result}л + ${truckSummary.qG_result}${gasUnit}`}</span></div>
                    <div className="flex justify-between text-[10px] md:text-sm opacity-80 font-medium"><span>Стоимость 1 км:</span><span>{truckSummary.kmG?.toFixed(2)} ₽</span></div>
                    <div className={`border-t ${truckTheme.border} pt-2 md:pt-3 flex justify-between font-bold ${truckTheme.textDark} text-sm md:text-2xl leading-none`}>
                      <span className="text-[8px] md:text-xs uppercase self-center font-bold tracking-tighter opacity-60">ИТОГО ЗА ГОД:</span>
                      <span className="font-black">{formatMoney(truckSummary.totalG)}</span>
                    </div>
                  </div>
                </div>
              </div>
              <div className="bg-slate-50 p-3 md:p-6 rounded-xl md:rounded-[2.5rem] border border-slate-100">
                <h4 className="text-[8px] md:text-[10px] font-bold text-slate-400 uppercase mb-2 md:mb-6 flex items-center gap-2 tracking-widest"><BarChart3 size={12}/> Структура затрат</h4>
                ...
                <div className="flex justify-between mt-2 text-[8px] md:text-[10px] font-bold uppercase tracking-tight">
                  {truckSubMode === 'REMOT' ? (<span className={`${truckTheme.textDark}`}>100% {gasNameStr} Метан</span>) : (
                    <><span className="text-red-700">ДИЗЕЛЬ: {formatMoney(truckSummary.totalG * (1 - truckInputs.substitutionRate/100))}</span><span className={`${truckTheme.textDark}`}>{gasNameStr}: {formatMoney(truckSummary.totalG * (truckInputs.substitutionRate/100))}</span></>
                  )}
                </div>
              </div>
