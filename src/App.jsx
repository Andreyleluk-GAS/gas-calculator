...
const App = () => {
  ...
  const passResults = useMemo(() => {
    ...
    const kmCostP = (v(fuelNorm) * passCoeffs.propane / 100) * v(pricePropane);
    const kmCostM = (v(fuelNorm) * passCoeffs.methane / 100) * v(priceMethane);
    ...
    const formatKm = (val) => val.toFixed(2).replace('.', ',');
    return {
  ...
  if (currentScreen === 'MAIN_SELECTION') {
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-2 md:p-4 font-sans text-slate-900 relative overflow-hidden">
        <div className="max-w-xl w-full relative z-10 flex flex-col items-center text-slate-900 text-slate-900">
          ...
          <div className="text-center mb-6 md:mb-10">
            <h1 className="text-2xl md:text-4xl font-extrabold mb-3 leading-tight">Калькулятор Экономии</h1>
          ...
          <div className="grid grid-cols-1 gap-2 md:gap-4 w-full">
            ...
            <button 
              ...
              className="bg-white/80 backdrop-blur-md p-4 md:p-6 rounded-[2rem] border-2 border-white/50 hover:border-blue-500 shadow-sm hover:shadow-xl transition-all group flex flex-col md:flex-row items-center md:gap-6 text-center md:text-left gap-2"
            >
            ...
            <button 
              ...
              className="bg-white/80 backdrop-blur-md p-4 md:p-6 rounded-[2rem] border-2 border-white/50 hover:border-emerald-500 shadow-sm hover:shadow-xl transition-all group flex flex-col md:flex-row items-center md:gap-6 text-center md:text-left gap-2"
            >
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center p-1 md:p-8 font-sans overflow-x-hidden text-slate-900">
        <div className="w-full max-w-lg lg:max-w-4xl flex flex-col gap-1 md:gap-3 text-slate-900">
          <header className="flex flex-col items-center text-center bg-white py-1.5 md:py-6 px-4 rounded-2xl shadow-sm border border-slate-200 w-full relative mt-0 md:mt-2 text-slate-900">
            <button onClick={() => window.history.back()} className="hidden md:block absolute left-2 top-2 md:left-3 md:top-4 p-2 text-slate-600 hover:bg-slate-50 rounded-full transition-colors"><ChevronLeft size={20} className="md:w-6 md:h-6" /></button>
            <h1 className="text-2xl md:text-3xl font-extrabold leading-tight text-slate-900">Топливный калькулятор</h1>
            <div className="flex items-center gap-2 mt-0.5 md:mt-2 text-slate-900">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-1 md:gap-3 w-full mb-1 md:mb-3 text-slate-900">
            {/* БЕНЗИН */}
            <div className="bg-white rounded-2xl p-2 md:p-5 shadow-sm border-t-4 border-amber-400 flex flex-col justify-between text-slate-900">
                <div>
                  ...
                  <div className="space-y-2 md:space-y-4 text-slate-900">
                      <div className="flex justify-between items-center bg-slate-50 p-1.5 md:p-2 rounded-xl border border-slate-100">
                          ...
                          <div className="flex items-baseline gap-1 text-slate-900 text-slate-900 text-slate-900">
                      ...
                      <div className="flex items-end justify-between border-t border-slate-100 pt-2 md:pt-3">
                         <div className="flex flex-col text-slate-900">
                            <p className="text-[9px] md:text-[10px] font-bold text-slate-400 uppercase mb-0.5 leading-none">Затраты / мес</p>
                            <p className="text-2xl md:text-3xl font-black leading-none text-slate-900">{formatMoney(passResults.costB)}</p>
                         </div>
                         <div className="flex flex-col text-right">
                            <p className="text-[9px] md:text-[10px] font-bold text-slate-400 uppercase mb-0.5 leading-none text-slate-400">1 км пробега</p>
                            <p className="text-lg md:text-xl font-bold text-slate-600 leading-none">{passResults.kmCostB} Р/км</p>
                         </div>
                      </div>
                  </div>
                </div>
                <p className="text-[10px] md:text-[11px] font-semibold text-slate-400 pt-1.5 border-t border-slate-100 mt-2 md:mt-4 text-center italic leading-none text-slate-900">Эталон для сравнения</p>
            </div>
            ...
            <div className="bg-white rounded-2xl p-2 md:p-5 shadow-sm border-t-4 border-emerald-500 bg-emerald-50/30 flex flex-col justify-between text-slate-900">
                  ...
                  <div className="space-y-2 md:space-y-4 text-slate-900 text-slate-900">
                      <div className="flex justify-between items-center bg-white p-1.5 md:p-2 rounded-xl border border-emerald-100 text-slate-900">
                          ...
                          <div className="flex items-baseline gap-1 text-slate-900">
                              <input type="number" name="pricePropane" value={passInputs.pricePropane} onChange={handlePassInputChange} className="w-14 bg-transparent text-right font-bold text-lg outline-none text-slate-900" />
                              <span className="text-[10px] font-bold text-emerald-600 text-slate-900">₽</span>
                          </div>
                      </div>
                      <div className="flex items-end justify-between border-t border-emerald-100/50 pt-2 md:pt-3 text-slate-900">
                         <div className="flex flex-col text-slate-900">
                            <p className="text-[9px] md:text-[10px] font-bold text-emerald-800/60 uppercase mb-0.5 leading-none">Затраты / мес</p>
                ...
                <p className="text-[10px] md:text-[12px] font-bold text-emerald-600 pt-1.5 border-t border-emerald-200 mt-2 md:mt-4 text-center tracking-tight leading-none text-center">Экономия в год: +{formatMoney(passResults.saveYearP)}</p>
            </div>

            {/* МЕТАН */}
            <div className="bg-white rounded-2xl p-2 md:p-5 shadow-sm border-t-4 border-blue-500 bg-blue-50/30 flex flex-col justify-between text-slate-900 text-slate-900">
                <div>
                  <div className="flex items-center gap-2 mb-1 md:mb-4">
                      <Gauge size={16} className="text-blue-500 md:w-5 md:h-5" />
                      <h3 className="text-xs font-bold uppercase tracking-wider text-blue-900 text-blue-900 text-blue-900">Метан</h3>
                  </div>
                  <div className="space-y-2 md:space-y-4 text-slate-900 text-slate-900 text-slate-900">
                      <div className="flex justify-between items-center bg-white p-1.5 md:p-2 rounded-xl border border-blue-100 text-slate-900">
                          <label className="text-[9px] md:text-[10px] font-bold text-blue-800 uppercase text-blue-800 text-blue-800 text-blue-800">Цена / м³</label>
                          <div className="flex items-baseline gap-1 text-slate-900">
                               <input type="number" name="priceMethane" value={passInputs.priceMethane} onChange={handlePassInputChange} className="w-14 bg-transparent text-right font-bold text-base md:text-lg outline-none text-slate-900" />
                               <span className="text-[10px] font-bold text-blue-600 text-slate-900 text-slate-900 text-slate-900">₽</span>
                          </div>
                      </div>
                      <div className="flex items-end justify-between border-t border-blue-100/50 pt-2 md:pt-3 text-slate-900 text-slate-900 text-slate-900">
                         <div className="flex flex-col text-slate-900 text-slate-900">
                            <p className="text-[9px] md:text-[10px] font-bold text-blue-800/60 uppercase mb-0.5 leading-none">Затраты / мес</p>
                            <p className="text-2xl md:text-3xl font-black leading-none text-blue-900 text-blue-900 text-blue-900">{formatMoney(passResults.costM)}</p>
                         </div>
                         <div className="flex flex-col text-right">
                            <p className="text-[9px] md:text-[10px] font-bold text-blue-800/60 uppercase mb-0.5 leading-none text-slate-400">1 км пробега</p>
                            <p className="text-lg md:text-xl font-bold text-blue-700 leading-none">{passResults.kmCostM} Р/км</p>
                         </div>
                      </div>
                  </div>
                </div>
                <p className="text-[10px] md:text-[12px] font-bold text-blue-600 pt-1.5 border-t border-blue-200 mt-2 md:mt-4 text-center tracking-tight leading-none text-center text-slate-900 text-slate-900 text-slate-900">Экономия в год: +{formatMoney(passResults.saveYearM)}</p>
            </div>
          </div>

          <div className="mt-auto text-slate-900 text-slate-900 text-slate-900 text-slate-900">
        ...
        {isPassSettingsOpen && (
          <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-3xl p-6 w-full max-sm shadow-2xl relative text-slate-900 text-slate-900">
              ...
              <div className="space-y-4 text-slate-900">
                ...
                <button onClick={()=>setIsPassSettingsOpen(false)} className="w-full bg-blue-600 text-white font-bold py-4 rounded-xl mt-4 shadow-lg uppercase tracking-wider text-xs active:scale-95 transition-all">Сохранить</button>
              </div>
            </div>
          </div>
        )}
  ...
  if (currentScreen === 'TRUCK_HOME') {
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4 font-sans text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900">
        <div className="max-w-4xl w-full text-slate-900 text-slate-900 text-slate-900 text-slate-900">
          ...
          <div className="text-center mb-8 text-slate-900">
            ...
            <p className="text-slate-600 font-medium uppercase tracking-tight font-bold text-slate-900 text-slate-900 text-slate-900">Выберите технологию переоборудования</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8 text-slate-900 text-slate-900 text-slate-900">
            <div onClick={() => { setTruckSubMode('GAS_DIESEL'); navigateTo('TRUCK_INPUTS'); }} className="bg-white p-6 rounded-[2rem] border-2 border-slate-100 hover:border-blue-500 cursor-pointer shadow-sm transition-all group">
              ...
              <h3 className="text-xl font-bold mb-1 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900">Газодизельный режим</h3>
              <p className="text-slate-500 text-sm text-slate-500">Частичное замещение ДТ метаном</p>
            </div>
            <div onClick={() => { setTruckSubMode('REMOT'); navigateTo('TRUCK_INPUTS'); }} className="bg-white p-6 rounded-[2rem] border-2 border-slate-100 hover:border-amber-400 cursor-pointer shadow-sm transition-all group text-slate-900 text-slate-900 text-slate-900">
              ...
              <h3 className="text-xl font-bold tracking-tight text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 tracking-tight text-slate-900">Ремоторизация ТС</h3>
  ...
  if (currentScreen === 'TRUCK_INPUTS') {
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col p-2 font-sans text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900">
        <div className="flex-1 flex flex-col items-center justify-center max-w-xl mx-auto w-full text-slate-900">
          <button onClick={() => window.history.back()} className="hidden md:flex items-center gap-1 mb-4 self-start text-slate-900 font-bold text-xs text-slate-900"><ChevronLeft size={18} /> Назад</button>
          <h1 className="text-xl font-bold mb-6 text-center uppercase tracking-tight font-sans text-slate-900">{isRem ? 'Ремоторизация ТС' : 'Газодизель ТС'}</h1>
          <div className="bg-white rounded-[2rem] shadow-xl p-5 w-full border border-slate-200">
            <div className="space-y-4 text-slate-900">
              <div className="grid grid-cols-2 gap-2 text-slate-900 text-slate-900 text-slate-900 text-slate-900">
              ...
              <div className="p-4 bg-red-50 rounded-2xl border border-red-100 text-slate-900 text-slate-900 text-slate-900 text-slate-900">
              ...
              <div className={`p-4 rounded-2xl border ${truckTheme.bg} ${truckTheme.border} text-slate-900 text-slate-900 text-slate-900 text-slate-900`}>
                <div className={`flex items-center gap-2 mb-3 ${truckTheme.textDark} font-bold uppercase text-[10px] text-slate-900 text-slate-900 text-slate-900 text-slate-900`}><Gauge size={14} /> Параметры газа</div>
              ...
              <div className={`grid ${!isRem ? 'grid-cols-2' : 'grid-cols-1'} gap-3 text-slate-900`}>
                <div>
                  ...
                  <input type="number" name="monthlyMileage" value={activeInp.monthlyMileage} onChange={(e) => handleTruckInputChange(e, isRem)} className="w-full p-3 border border-slate-300 rounded-xl font-bold text-sm text-slate-900 text-slate-900 text-slate-900" />
                </div>
                {!isRem && (
                  <div>
  ...
  if (currentScreen === 'TRUCK_REPORT') {
    return (
      <div className="min-h-screen bg-slate-50 p-1.5 md:p-8 font-sans text-slate-900 overflow-x-hidden text-slate-900 text-slate-900 text-slate-900">
        <div className="max-w-6xl mx-auto text-slate-900 text-slate-900 text-slate-900">
          <header className="mb-2 md:mb-4 flex items-center justify-between print-hidden text-slate-900 text-slate-900">
            <button onClick={() => window.history.back()} className="hidden md:flex items-center gap-1 px-4 py-2 bg-white border border-slate-300 rounded-xl text-xs font-bold text-slate-800 shadow-sm font-sans text-slate-900 text-slate-900"><ChevronLeft size={14} /> Назад</button>
          ...
          <div className="bg-white p-3 md:p-10 rounded-2xl md:rounded-[2.5rem] shadow-sm border border-slate-200 relative overflow-hidden text-slate-900 text-slate-900 text-slate-900">
             <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-0 opacity-5 select-none text-slate-900">
            ...
            <div className="relative z-10 text-slate-900 text-slate-900 text-slate-900 text-slate-900">
              <div className="mb-3 md:mb-8 text-center border-b border-slate-100 pb-2 md:pb-6 font-sans text-slate-900 text-slate-900 text-slate-900">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 gap-1.5 md:gap-3 mb-3 md:mb-8 font-bold text-slate-900 text-slate-900 text-slate-900">
                ...
                <div className="p-2 md:p-3 bg-slate-50 rounded-xl border border-slate-200 font-sans text-slate-900 text-slate-900 text-slate-900 text-slate-900"><div className="text-slate-700 text-[8px] md:text-[10px] uppercase mb-0.5 md:mb-1 font-bold text-slate-900 text-slate-900 text-slate-900">Расход ДТ</div><div className="font-bold text-[10px] md:text-base text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900">{truckSummary.qD_base} л/100км</div></div>
              ...
              <div className={`grid grid-cols-1 ${systemType === 'cng' ? 'md:grid-cols-2' : ''} gap-2 md:gap-6 mb-3 md:mb-8 font-sans text-slate-900 text-slate-900 text-slate-900`}>
                <div className={`bg-gradient-to-br ${truckTheme.gradient} text-white p-4 md:p-8 rounded-2xl shadow-xl flex flex-row justify-between relative overflow-hidden font-sans text-white text-white text-white`}>
                  <div className="relative z-10 flex flex-col justify-between w-2/3 md:w-3/4 text-white text-white text-white text-white text-white text-white">
                    <div><div className="text-[9px] md:text-xs font-bold uppercase tracking-wider opacity-90 font-sans text-white text-white text-white text-white text-white text-white">Экономия (Базовый)</div><div className="text-xl md:text-5xl font-bold mb-2 md:mb-4 leading-tight font-sans text-white text-white text-white text-white text-white text-white text-white text-white">{formatMoney(truckSummary.savings)}</div></div>
                    <div className="flex gap-2 md:gap-3 flex-wrap text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white"><div className="bg-white/20 px-2 py-0.5 md:px-3 md:py-1.5 rounded-lg text-[8px] md:text-[10px] font-bold uppercase text-white">{formatMoney(truckSummary.monthlySav)} / мес</div><div className="bg-white/20 px-2 py-0.5 md:px-3 md:py-1.5 rounded-lg text-[8px] md:text-[10px] font-bold uppercase font-sans text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white">- {Math.round((truckSummary.savings / (truckSummary.totalD || 1)) * 100)}% затрат</div></div>
                  </div>
                  <div className="absolute right-[10px] top-[10px] bottom-[10px] flex items-center justify-end w-2/5 md:w-1/3 select-none text-white text-white text-white text-white text-white text-white text-white">
                    <img src="/logo.png" alt="Logo" className="h-full w-auto object-contain object-right opacity-30 text-white text-white text-white text-white text-white text-white text-white text-white" onError={(e)=>{e.target.style.display='none'}} />
                  </div>
                </div>
                {systemType === 'cng' && (
                  <div className="bg-white border-2 border-blue-100 p-4 md:p-8 rounded-2xl md:rounded-[2rem] shadow-lg flex flex-col justify-between relative overflow-hidden text-slate-900 font-sans text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900">
                    <div className="absolute top-0 right-0 p-1.5 md:p-3 bg-blue-700 text-white rounded-bl-xl md:rounded-bl-3xl font-bold text-[8px] md:text-[10px] uppercase z-20 font-sans text-white text-white text-white text-white text-white text-white text-white">Программа ГГМТ</div>
                    <div className="relative z-10 w-2/3 md:w-3/4 mt-2 font-bold text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900">
                        <div><div className="text-[9px] md:text-xs font-bold text-slate-700 mb-0.5 md:mb-1 flex items-center gap-1 uppercase tracking-wider font-sans text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900"><Tag size={10} className="text-blue-600 font-sans text-blue-600 text-blue-600 text-blue-600" /> Со скидкой на метан 20%</div><div className="text-xl md:text-5xl font-bold text-blue-900 mb-2 md:mb-4 leading-tight text-blue-900 text-blue-900 text-blue-900 text-blue-900 text-blue-900 text-blue-900 text-blue-900">{formatMoney(truckSummary.savingsDiscounted)}</div></div>
                        <div className="flex gap-2 md:gap-3 flex-wrap text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900"><div className="bg-blue-50 text-blue-900 px-2 py-0.5 md:px-3 md:py-1.5 rounded-lg text-[8px] md:text-[10px] font-bold border border-blue-200 uppercase font-sans text-blue-900 text-blue-900 text-blue-900 text-blue-900 text-blue-900 text-blue-900 text-blue-900 text-blue-900 text-blue-900 text-blue-900">{formatMoney(truckSummary.monthlySavDiscounted)} / мес</div><div className="bg-blue-50 text-blue-900 px-2 py-0.5 md:px-3 md:py-1.5 rounded-lg text-[8px] md:text-[10px] font-bold border border-blue-200 uppercase font-sans text-blue-900 text-blue-900 text-blue-900 text-blue-900 text-blue-900 text-blue-900 text-blue-900 text-blue-900 text-blue-900 text-blue-900">- {Math.round((truckSummary.savingsDiscounted / (truckSummary.totalD || 1)) * 100)}% затрат</div></div>
                    </div>
                    <div className="absolute right-[10px] bottom-[10px] flex items-end justify-end max-h-[35%] w-[28%] select-none opacity-40 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2 md:gap-6 mb-3 md:mb-8 font-sans font-bold text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900">
                <div className="border border-red-200 rounded-xl md:rounded-[2rem] p-3 md:p-6 bg-red-50/30 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900">
                  <div className="text-red-900 font-bold text-[10px] md:text-xs uppercase mb-2 md:mb-4 flex items-center gap-2 font-sans text-red-900 text-red-900 text-red-900 text-red-900 text-red-900 text-red-900 text-red-900"><Fuel size={12}/> На дизеле (100%)</div>
                  <div className="space-y-1.5 md:space-y-3 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900">
                    <div className="flex justify-between text-[10px] md:text-sm font-bold opacity-80 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900"><span>Расход на 100км:</span><span>{truckSummary.qD_base} л</span></div>
                    <div className="flex justify-between text-[10px] md:text-sm font-bold opacity-80 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900"><span>Стоимость 1 км:</span><span>{truckSummary.kmD?.toFixed(2)} ₽</span></div>
                    <div className="border-t border-red-200 pt-2 md:pt-3 flex justify-between font-bold text-red-700 text-sm md:text-2xl leading-none text-red-700 text-red-700 text-red-700 text-red-700 text-red-700 text-red-700 text-red-700 text-red-700 text-red-700">
                      <span className="text-[8px] md:text-xs uppercase self-center font-bold text-red-600 tracking-tighter text-red-600 text-red-600 text-red-600 text-red-600 text-red-600 text-red-600 text-red-600 text-red-600 text-red-600 text-red-600">ИТОГО ЗА ГОД:</span>
                      <span className="font-black text-red-800 text-red-800 text-red-800 text-red-800 text-red-800 text-red-800 text-red-800 text-red-800 text-red-800 text-red-800 text-red-800">{formatMoney(truckSummary.totalD)}</span>
                    </div>
                  </div>
                </div>
                <div className={`border ${truckTheme.border} rounded-xl md:rounded-[2rem] p-3 md:p-6 ${truckTheme.bg}/30 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900`}>
                  <div className={`${truckTheme.textDark} font-bold text-[10px] md:text-xs uppercase mb-2 md:mb-4 flex items-center gap-2 font-sans text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900`}>{isLngMode?<Flame size={12}/>:<Gauge size={12}/>} {truckSubMode === 'REMOT' ? `На газе (${gasNameStr} 100%)` : `Газодизель (${truckInputs.substitutionRate}% замещения)`}</div>
                  <div className="space-y-1.5 md:space-y-3 font-bold text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900">
                    <div className="flex justify-between text-[10px] md:text-sm opacity-80 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900"><span>На 100км:</span><span>{truckSubMode === 'REMOT' ? `${truckSummary.qG_result} ${gasUnit}` : `${truckSummary.qD_result}л + ${truckSummary.qG_result}${gasUnit}`}</span></div>
                    <div className="flex justify-between text-[10px] md:text-sm opacity-80 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900"><span>Стоимость 1 км:</span><span>{truckSummary.kmG?.toFixed(2)} ₽</span></div>
                    <div className={`border-t ${truckTheme.border} pt-2 md:pt-3 flex justify-between font-bold ${truckTheme.textDark} text-sm md:text-2xl leading-none text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900`}>
                      <span className="text-[8px] md:text-xs uppercase self-center font-bold tracking-tighter text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900">ИТОГО ЗА ГОД:</span>
                      <span className="font-black text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900">{formatMoney(truckSummary.totalG)}</span>
                    </div>
                  </div>
                </div>
              </div>
              <div className="bg-slate-100 p-3 md:p-6 rounded-xl md:rounded-[2.5rem] border border-slate-200 font-bold text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900">
                <h4 className="text-[8px] md:text-[10px] font-bold text-slate-500 uppercase mb-2 md:mb-6 flex items-center gap-2 tracking-widest font-sans text-slate-500 text-slate-500 text-slate-500 text-slate-500 text-slate-500 text-slate-500 text-slate-500 text-slate-500 text-slate-500 text-slate-500 text-slate-500"><BarChart3 size={12}/> Структура затрат</h4>
                <div className="h-6 md:h-10 w-full bg-slate-300 rounded-lg md:rounded-2xl overflow-hidden flex shadow-inner text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900">
                  {truckSubMode === 'REMOT' ? (<div className={`${truckTheme.button} h-full w-full text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900`}></div>) : (
                    <>
                      <div className="bg-red-600 h-full transition-all duration-1000 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900" style={{ width: `${(100 - truckInputs.substitutionRate)}%` }}></div>
                      <div className={`${truckTheme.button} h-full transition-all duration-1000 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900`} style={{ width: `${truckInputs.substitutionRate}%` }}></div>
                    </>
                  )}
                </div>
                <div className="flex justify-between mt-2 text-[8px] md:text-[10px] font-bold uppercase tracking-tight text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900">
                  {truckSubMode === 'REMOT' ? (<span className={`${truckTheme.textDark} font-bold text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900`}>100% {gasNameStr} Метан</span>) : (
                    <>
                      <span className="text-red-700 font-bold text-red-700 text-red-700 text-red-700 text-red-700 text-red-700 text-red-700 text-red-700 text-red-700 text-red-700 text-red-700 text-red-700 text-red-700 text-red-700 text-red-700 text-red-700">ДИЗЕЛЬ: {formatMoney(truckSummary.totalG * (1 - truckInputs.substitutionRate/100))}</span>
                      <span className={`${truckTheme.textDark} font-bold text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900`}>{gasNameStr}: {formatMoney(truckSummary.totalG * (truckInputs.substitutionRate/100))}</span>
                    </>
                  )}
                </div>
              </div>
