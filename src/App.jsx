...
const App = () => {
  ...
  const AppFooter = ({ showDisclaimer = false }) => (
    <footer className="mt-8 mb-4 flex flex-col items-center text-center gap-1.5 font-sans text-slate-900">
      ...
      <a href="https://t.me/Le_luk" target="_blank" rel="noopener noreferrer" className="mt-2 flex items-center gap-1.5 px-3 py-1 bg-[#0088cc] text-white rounded-full text-[10px] md:text-xs font-bold hover:bg-[#0077b3] shadow-sm active:scale-95 transition-all text-white"><Send size={12} fill="white" /> Написать в Telegram</a>
  ...
  if (currentScreen === 'MAIN_SELECTION') {
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-2 md:p-4 font-sans text-slate-900 relative overflow-hidden text-slate-900 text-slate-900">
        <div className="max-w-xl w-full relative z-10 flex flex-col items-center">
          ...
          <div className="grid grid-cols-1 gap-2 md:gap-4 w-full text-slate-900">
            {/* ГРУЗОВОЕ ТС */}
            <button 
            ...
            >
              ...
              <div className="flex flex-col items-center md:items-start overflow-hidden text-slate-900">
  ...
  if (currentScreen === 'PASSENGER_CALC') {
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center p-1 md:p-8 font-sans overflow-x-hidden text-slate-900">
        <div className="w-full max-w-lg lg:max-w-4xl flex flex-col gap-1 md:gap-3 text-slate-900 text-slate-900 text-slate-900 text-slate-900">
          ...
          <section className="grid grid-cols-2 gap-1 md:gap-3 w-full text-slate-900 text-slate-900">
            <div className="bg-white p-1.5 md:p-4 rounded-xl shadow-sm border border-slate-200">
                <label className="block text-[9px] md:text-[10px] font-bold text-slate-700 uppercase mb-0.5 tracking-tight text-slate-700">Пробег (мес)</label>
            ...
            <div className="bg-white p-1.5 md:p-4 rounded-xl shadow-sm border border-slate-200 text-slate-900">
                <label className="block text-[9px] md:text-[10px] font-bold text-slate-700 uppercase mb-0.5 tracking-tight text-slate-700">Расход бензина</label>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-1 md:gap-3 w-full mb-1.5 md:mb-3 text-slate-900">
            {/* БЕНЗИН */}
            <div className="bg-amber-50 rounded-2xl p-2 md:p-4 shadow-sm border-t-4 border-amber-400 flex flex-col justify-between text-slate-900">
                <div>
                  <div className="flex items-center gap-2 mb-1.5 md:mb-4 text-slate-900">
                      <Fuel size={16} className="text-amber-500" />
                      <h3 className="text-xs font-bold uppercase tracking-wider text-amber-900">Бензин</h3>
                  </div>
                  <div className="space-y-2 md:space-y-4 text-slate-900">
                      <div className="flex justify-between items-center bg-white p-1.5 md:p-2 rounded-xl border border-amber-200 text-slate-900">
                          <label className="text-[9px] md:text-[10px] font-bold text-amber-700 uppercase">Цена/литр</label>
                          <div className="flex items-baseline gap-1">
                              <input type="number" name="priceBenzin" value={passInputs.priceBenzin} onChange={handlePassInputChange} className="w-14 bg-transparent text-right font-bold text-lg outline-none text-amber-900" />
                              <span className="text-[10px] font-bold text-amber-400">₽</span>
                          </div>
                      </div>
                      <div className="flex items-end justify-between border-t border-amber-100 pt-2 md:pt-3">
                         <div className="flex flex-col">
                            <p className="text-[9px] md:text-[10px] font-bold text-amber-600 uppercase mb-0.5 leading-none">Затраты / мес</p>
                            <p className="text-2xl md:text-3xl font-black leading-none text-amber-900">{formatMoney(passResults.costB)}</p>
                         </div>
                ...
                <p className="text-[14px] md:text-[15px] font-semibold text-amber-700/60 pt-1.5 border-t border-amber-100 mt-2.5 md:mt-4 text-center italic leading-none">Эталон для сравнения</p>
            </div>

            {/* ПРОПАН */}
            <div className="bg-emerald-50 rounded-2xl p-2 md:p-4 shadow-sm border-t-4 border-emerald-500 flex flex-col justify-between text-slate-900">
                <div>
                  ...
                  <div className="space-y-2 md:space-y-4 text-slate-900">
                      <div className="flex justify-between items-center bg-white p-1.5 md:p-2 rounded-xl border border-emerald-200">
                      ...
                      <div className="flex items-end justify-between border-t border-emerald-200 pt-2 md:pt-3">
                         <div className="flex flex-col">
                ...
                <p className="text-[13px] md:text-[15px] font-bold text-emerald-600 pt-1.5 border-t border-emerald-200 mt-2.5 md:mt-4 text-center tracking-tight leading-none">Экономия в год: +{formatMoney(passResults.saveYearP)}</p>
            </div>

            {/* МЕТАН */}
            <div className="bg-blue-50 rounded-2xl p-2 md:p-4 shadow-sm border-t-4 border-blue-500 flex flex-col justify-between text-slate-900">
                <div>
                  <div className="flex items-center gap-2 mb-1.5 md:mb-4">
                  ...
                  <div className="space-y-2 md:space-y-4 text-slate-900 text-slate-900">
                      <div className="flex justify-between items-center bg-white p-1.5 md:p-2 rounded-xl border border-blue-200 text-slate-900">
                          <label className="text-[9px] md:text-[10px] font-bold text-blue-800 uppercase text-blue-800">Цена / м³</label>
                      ...
                      <div className="flex items-end justify-between border-t border-blue-200 pt-2 md:pt-3">
                         <div className="flex flex-col">
                ...
                <p className="text-[13px] md:text-[15px] font-bold text-blue-600 pt-1.5 border-t border-blue-200 mt-2.5 md:mt-4 text-center tracking-tight leading-none text-slate-900 text-slate-900">Экономия в год: +{formatMoney(passResults.saveYearM)}</p>
            </div>
          </div>
        ...
        {isPassSettingsOpen && (
          <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-3xl p-6 w-full max-sm shadow-2xl relative text-slate-900">
              ...
              <h2 className="text-xl font-bold mb-6 text-slate-900 text-slate-900">Настройки ГБО</h2>
  ...
  if (currentScreen === 'TRUCK_HOME') {
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4 font-sans text-slate-900">
        <div className="max-w-4xl w-full text-slate-900">
          <button onClick={() => window.history.back()} className="flex items-center gap-1 mb-6 text-slate-900 font-bold text-sm hover:opacity-70 text-slate-900">
            <ChevronLeft size={20} />
            <span>Назад</span>
          </button>
          <div className="text-center mb-8">
            <h1 className="text-2xl md:text-4xl font-extrabold mb-2 text-slate-900 text-slate-900">Грузовой транспорт</h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8 text-slate-900 text-slate-900">
            <div onClick={() => { setTruckSubMode('GAS_DIESEL'); navigateTo('TRUCK_INPUTS'); }} className="bg-white p-6 rounded-[2rem] border-2 border-slate-100 hover:border-blue-500 cursor-pointer shadow-sm transition-all group">
              ...
              <h3 className="text-xl font-bold mb-1 text-slate-900 text-slate-900">Газодизельный режим</h3>
            ...
            <div onClick={() => { setTruckSubMode('REMOT'); navigateTo('TRUCK_INPUTS'); }} className="bg-white p-6 rounded-[2rem] border-2 border-slate-100 hover:border-amber-400 cursor-pointer shadow-sm transition-all group">
              ...
              <h3 className="text-xl font-bold tracking-tight text-slate-900 text-slate-900">Ремоторизация ТС</h3>
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col p-2 font-sans text-slate-900">
        <div className="flex-1 flex flex-col items-center justify-center max-w-xl mx-auto w-full text-slate-900">
          <button onClick={() => window.history.back()} className="flex items-center gap-1 mb-4 self-start text-slate-900 font-bold text-xs text-slate-900 text-slate-900">
            <ChevronLeft size={18} />
            <span>Назад</span>
          </button>
          ...
          <div className="bg-white rounded-[2rem] shadow-xl p-5 w-full border border-slate-200">
            <div className="space-y-4 text-slate-900">
              ...
              <div className={`p-4 rounded-2xl border ${truckTheme.bg} ${truckTheme.border} text-slate-900 text-slate-900`}>
  ...
  if (currentScreen === 'TRUCK_REPORT') {
    return (
      <div className="min-h-screen bg-slate-50 p-1.5 md:p-8 font-sans text-slate-900 overflow-x-hidden text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900">
        <div className="max-w-6xl mx-auto text-slate-900 text-slate-900 text-slate-900 text-slate-900">
          <header className="mb-2 md:mb-4 flex items-center justify-between print-hidden text-slate-900">
            <button onClick={() => window.history.back()} className="flex items-center gap-1 px-4 py-2 bg-white border border-slate-300 rounded-xl text-xs font-bold text-slate-800 shadow-sm font-sans text-slate-900">
                <ChevronLeft size={14} />
                <span>Назад</span>
            </button>
            <button onClick={() => window.print()} className="hidden md:flex items-center gap-2 px-5 py-2 bg-white border border-slate-300 rounded-xl text-sm font-bold text-slate-900 shadow-sm font-sans text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900"><Printer size={16} /> Печать</button>
          </header>

          <div className="bg-white p-3 md:p-10 rounded-2xl md:rounded-[2.5rem] shadow-sm border border-slate-200 relative overflow-hidden text-slate-900">
             <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-0 opacity-5 select-none text-slate-900">
                 <img src="/logo.png" alt="Watermark" className="w-3/4 md:w-1/2 object-contain transform -rotate-12 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900" onError={(e) => { e.target.style.display = 'none'; }} />
            </div>
            <div className="relative z-10 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900">
              <div className="mb-3 md:mb-8 text-center border-b border-slate-100 pb-2 md:pb-6 font-sans text-slate-900 text-slate-900 text-slate-900 text-slate-900">
                <h1 className="text-sm md:text-2xl font-bold uppercase tracking-tight leading-tight text-slate-900 text-slate-900">
                  {truckSubMode === 'REMOT' ? (
              ...
              <div className={`grid grid-cols-1 ${systemType === 'cng' ? 'md:grid-cols-2' : ''} gap-2 md:gap-6 mb-3 md:mb-8 font-sans text-slate-900 text-slate-900 text-slate-900`}>
                <div className={`bg-gradient-to-br ${truckTheme.gradient} text-white p-4 md:p-8 rounded-2xl shadow-xl flex flex-row justify-between relative overflow-hidden font-sans text-white text-white`}>
                  <div className="relative z-10 flex flex-col justify-between w-2/3 md:w-3/4 text-white text-white text-white text-white text-white">
                    <div><div className="text-[9px] md:text-xs font-bold uppercase tracking-wider opacity-90 font-sans text-white text-white text-white">Экономия (Базовый)</div><div className="text-xl md:text-5xl font-bold mb-2 md:mb-4 leading-tight font-sans text-white text-white text-white text-white text-white text-white text-white">{formatMoney(truckSummary.savings)}</div></div>
                    <div className="flex gap-2 md:gap-3 flex-wrap text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white"><div className="bg-white/20 px-2 py-0.5 md:px-3 md:py-1.5 rounded-lg text-[8px] md:text-[10px] font-bold uppercase text-white">{formatMoney(truckSummary.monthlySav)} / мес</div><div className="bg-white/20 px-2 py-0.5 md:px-3 md:py-1.5 rounded-lg text-[8px] md:text-[10px] font-bold uppercase font-sans text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white">- {Math.round((truckSummary.savings / (truckSummary.totalD || 1)) * 100)}% затрат</div></div>
                  </div>
                  <div className="absolute right-[10px] top-[10px] bottom-[10px] flex items-center justify-end w-2/5 md:w-1/3 select-none text-white text-white text-white text-white text-white text-white text-white text-white">
                    <img src="/logo.png" alt="Logo" className="h-full w-auto object-contain object-right opacity-30 text-white text-white text-white text-white text-white text-white text-white text-white text-white" onError={(e)=>{e.target.style.display='none'}} />
                  </div>
                </div>
                {systemType === 'cng' && (
                  <div className="bg-white border-2 border-blue-100 p-4 md:p-8 rounded-2xl md:rounded-[2rem] shadow-lg flex flex-col justify-between relative overflow-hidden text-slate-900 font-sans text-slate-900 text-slate-900 text-slate-900 text-slate-900">
                    <div className="absolute top-0 right-0 p-1.5 md:p-3 bg-blue-700 text-white rounded-bl-xl md:rounded-bl-3xl font-bold text-[8px] md:text-[10px] uppercase z-20 font-sans text-white text-white text-white text-white">Программа ГГМТ</div>
                    <div className="relative z-10 w-2/3 md:w-3/4 mt-2 font-bold text-slate-900 text-slate-900 text-slate-900 text-slate-900">
                        <div><div className="text-[9px] md:text-xs font-bold text-slate-700 mb-0.5 md:mb-1 flex items-center gap-1 uppercase tracking-wider font-sans text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900"><Tag size={10} className="text-blue-600 font-sans text-blue-600 text-blue-600 text-blue-600" /> Со скидкой на метан 20%</div><div className="text-xl md:text-5xl font-bold text-blue-900 mb-2 md:mb-4 leading-tight text-blue-900 text-blue-900 text-blue-900 text-blue-900 text-blue-900 text-blue-900 text-blue-900 text-blue-900 text-blue-900">{formatMoney(truckSummary.savingsDiscounted)}</div></div>
                        <div className="flex gap-2 md:gap-3 flex-wrap text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900"><div className="bg-blue-50 text-blue-900 px-2 py-0.5 md:px-3 md:py-1.5 rounded-lg text-[8px] md:text-[10px] font-bold border border-blue-200 uppercase font-sans text-blue-900 text-blue-900 text-blue-900 text-blue-900 text-blue-900 text-blue-900 text-blue-900 text-blue-900 text-blue-900 text-blue-900 text-blue-900 text-blue-900">- {Math.round((truckSummary.savingsDiscounted / (truckSummary.totalD || 1)) * 100)}% затрат</div></div>
                    </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2 md:gap-6 mb-3 md:mb-8 font-sans font-bold text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900">
                <div className="border border-red-200 rounded-xl md:rounded-[2rem] p-3 md:p-6 bg-red-50/30 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900">
                  <div className="text-red-900 font-bold text-[10px] md:text-xs uppercase mb-2 md:mb-4 flex items-center gap-2 font-sans text-red-900 text-red-900 text-red-900 text-red-900 text-red-900"><Fuel size={12}/> На дизеле (100%)</div>
                  <div className="space-y-1.5 md:space-y-3 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900">
                    ...
                    <div className="border-t border-red-200 pt-2 md:pt-3 flex justify-between font-bold text-red-700 text-sm md:text-2xl leading-none text-red-700 text-red-700 text-red-700 text-red-700 text-red-700 text-red-700 text-red-700 text-red-700 text-red-700 text-red-700">
                      <span className="text-[8px] md:text-xs uppercase self-center font-bold text-red-600 tracking-tighter text-red-600 text-red-600 text-red-600 text-red-600 text-red-600 text-red-600 text-red-600 text-red-600 text-red-600 text-red-600 text-red-600">ИТОГО ЗА ГОД:</span>
                      <span className="font-black text-red-800 text-red-800 text-red-800 text-red-800 text-red-800 text-red-800 text-red-800 text-red-800">{formatMoney(truckSummary.totalD)}</span>
                    </div>
                  </div>
                </div>
                <div className={`border ${truckTheme.border} rounded-xl md:rounded-[2rem] p-3 md:p-6 ${truckTheme.bg}/30 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900`}>
                  ...
                  <div className="space-y-1.5 md:space-y-3 font-bold text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900">
                    ...
                    <div className={`border-t ${truckTheme.border} pt-2 md:pt-3 flex justify-between font-bold ${truckTheme.textDark} text-sm md:text-2xl leading-none text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900`}>
                      <span className="text-[8px] md:text-xs uppercase self-center font-bold tracking-tighter text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900">ИТОГО ЗА ГОД:</span>
              ...
              <div className="bg-slate-100 p-3 md:p-6 rounded-xl md:rounded-[2.5rem] border border-slate-200 font-bold text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900">
                <h4 className="text-[8px] md:text-[10px] font-bold text-slate-500 uppercase mb-2 md:mb-6 flex items-center gap-2 tracking-widest font-sans text-slate-500 text-slate-500 text-slate-500 text-slate-500 text-slate-500 text-slate-500 text-slate-500 text-slate-500 text-slate-500 text-slate-500 text-slate-500 text-slate-500"><BarChart3 size={12}/> Структура затрат</h4>
                <div className="h-6 md:h-10 w-full bg-slate-300 rounded-lg md:rounded-2xl overflow-hidden flex shadow-inner text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900">
                  {truckSubMode === 'REMOT' ? (<div className={`${truckTheme.button} h-full w-full text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900`}></div>) : (
                    <>
                      <div className="bg-red-600 h-full transition-all duration-1000 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900" style={{ width: `${(100 - truckInputs.substitutionRate)}%` }}></div>
                ...
                <div className="flex justify-between mt-2 text-[8px] md:text-[10px] font-bold uppercase tracking-tight text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900">
                  {truckSubMode === 'REMOT' ? (<span className={`${truckTheme.textDark} font-bold text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900`}>100% {gasNameStr} Метан</span>) : (
                    <>
